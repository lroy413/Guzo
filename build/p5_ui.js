  ...
  if (S.active) {
    ...
    html += `<div class="hero">
      ...
      <div class="hero-in">
        ...
        ${/* What is actually in it, before you commit to it.

              The card said "Full body · 60 min" and put the movements behind
              the Start button, which is the one thing on this screen somebody
              wants to know before deciding whether today is a training day.
              Generated by the real engine through previewSession(), so it is
              the session you will get and not an illustration of one — and so
              it costs nothing in state, which drawing it any other way would.

              A routine already lists its own movements when you open it, so
              this is only for a generated session. */''}
        ${rt ? '' : (() => {
          const prev = previewSession(type, c.env, suggested, null);
          const items = (prev && prev.exercises) || [];
          if (!items.length) return '';
          return `<div class="hero-what">
            ${items.map(it => {
              const ex = EX[it.exId];
              if (!ex) return '';
              const dose = it.cardio ? `${it.targetR} min`
                : it.load === 'time' ? `${it.sets.length} × ${it.targetR}s`
                : `${it.sets.length} × ${it.targetR}`;
              return `<div class="hw-row">
                <span class="hw-n">${h(ex.name)}</span>
                <span class="hw-d mono">${dose}</span>
              </div>`;
            }).join('')}
          </div>`;
        })()}

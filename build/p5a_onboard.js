...
const GOALS = {
  ...
  strength: { k:'strength', ico:'goal-strength', label:'Get stronger',
              note:'Three-minute rests on the main lifts, so a heavy set is actually heavy.' },
  muscle:   { k:'muscle',   ico:'goal-muscle', label:'Build muscle',
              note:'Protein at 1.9 g per kg, and spare minutes at the end of a session become extra accessory sets.' },
  ...
  lean:     { k:'lean',     ico:'goal-lean', label:'Lose fat',
              note:'Calories below maintenance, protein at 2.2 g per kg to hold the muscle you have. Training stays heavy — in a deficit that is what keeps it.' },
  ...
  consist:  { k:'consist',  ico:'goal-consist', label:'Just be consistent',
              note:'Nothing extra on top. The app already spends its effort fitting the day you actually got.' }
};
...
const OB_STEPS = [
  ...
  { k:'diet',        q:20, ch:4 },
  { k:'seed',        q:21, ch:4 },
  /* The payoff, and it is placed here for two reasons. It comes after `seed`
     so the weights it shows are the ones the user just confirmed rather than
     numbers that change a screen later; and it comes immediately before
     `ready`, which is the one screen that asks for more work at the moment
     somebody expects to be finished. Twenty questions of giving with a single
     screen giving anything back is what this is for.

     Not a question, so it does not carry a `q` and does not move the counter. */
  { k:'preview' },
  { k:'ready' }
];
...
OB_RENDER.age = () => {
  ...
  return obShell(`
    ...
    <p class="ob-sub">It changes how hard sessions are spaced and the floor under your protein.
      It does not touch your readiness score, the size ladder or a rep range &mdash;
      there is no evidence behind those and inventing some would be worse than
      not asking.</p>
    <div class="field mb">
    ...
    <p class="ob-why">The evidence on recovery is genuinely split. What it does
      support is the practical end &mdash; 48 to 72 hours between hard sessions &mdash; so
      the app says that where it applies and leaves your week
...
OB_RENDER.goals = () => obShell(`
  ...
  <p class="ob-sub">Pick everything that applies. This sets your protein, your calories, your accessory work, and your rest between heavy sets.</p>
  <div class="opts">
...
OB_RENDER.level = () => obShell(`
  ...
  <p class="ob-sub">Be honest rather than optimistic. It only decides where you start, and starting light is far easier to fix.</p>
  <div class="opts">
...
OB_RENDER.priorities = () => obShell(`
  ...
  <p class="ob-sub">Pick up to three. Each earns an extra set in every full session, rotating so no one area hogs the time.</p>
  <div class="opts">
  ...
  <p class="ob-why">Leave it empty if nothing stands out &mdash; a balanced plan is a perfectly good plan.</p>
...
OB_RENDER.cardio = () => obShell(`
  ...
  <p class="ob-sub">Added as a finisher on the end of strength sessions, so it never becomes a separate thing to find time for.</p>
  <div class="opts">
...
OB_RENDER.injuries = () => {
  ...
  return obShell(`
    ...
    <p class="ob-sub">Old injuries and current niggles both count. Anything you flag is removed from the library for good and will never be given to you.</p>
    <div class="opts">
...
OB_RENDER.structure = () => {
  ...
  const plain = {
    anchor3:'Three full-body sessions. Every one works everything, so a missed day never leaves a gap. The most forgiving there is.',
    ul4:'Four sessions alternating upper and lower. More volume per muscle, but a missed day leaves half of you untrained.',
    ul5:'Push, pull, legs, then upper and lower. Everything trained close to twice a week, which is where the evidence points.',
    bro5:'One area a day. Easy to remember, but each muscle is trained once a week &mdash; so a missed day costs you that muscle.',
    ppl6:'Six sessions across push, pull and legs. The strongest stimulus there is, and the least forgiving of a chaotic week.',
    hold2:'Two full-body sessions. Not growth &mdash; a holding pattern for heavy blocks, and comfortably ahead of losing ground.'
  };
  return obShell(`
    <h1 class="ob-q">How many days a week, realistically?</h1>
    <p class="ob-sub">Not the number you wish you could do &mdash; the number you hit in an ordinary month. Changeable any time.</p>
    <div class="opts">
...
OB_RENDER.length = () => {
  ...
  return obShell(`
    ...
    <p class="ob-sub">The most load-bearing number in the app. Every other duration comes off it &mdash; a busy day, a clear one, and what gets cut when you are short.</p>
    <div class="opts">
...
/* ---------- ready ---------- */
/* ---------- the first session, built from the answers so far ----------
   Generated by the real engine rather than mocked up, because a preview that
   is not the thing is a promise rather than a payoff — and because a mock
   would go stale the first time session generation changed.

   The engine reads S, not the draft, so the draft is applied to S and the
   whole of S is put back afterwards. A snapshot round-trip rather than
   unwinding field by field: generateSession() also lazily writes rep targets
   into S.lifts through targetFor(), which is easy to miss and would leave a
...
function obPreviewSession() {
  ...
  try {
    ...
    if (obDraft.bodyweight > 0) P.bodyweight = [{ d: today(), w: obDraft.bodyweight }];
...
OB_RENDER.preview = () => {
  const prog = PROGRAMS[obDraft.programId] || PROGRAMS.anchor3;
  const sess = obPreviewSession();
  const items = (sess && sess.exercises) || [];
  const u = obDraft.units;

  const row = (it) => {
    const ex = EX[it.exId];
    if (!ex) return '';
    const reps = it.cardio ? `${it.targetR} min`
               : it.load === 'time' ? `${it.sets.length} × ${it.targetR}s`
               : `${it.sets.length} × ${it.targetR}`;
    const w = (!it.cardio && it.targetW != null && it.targetW !== '') ? `${fmtW(it.targetW)}${u}` : '';
    return `<div class="prev-row">
      <div class="grow"><div class="prev-n">${h(ex.name)}</div>
        ${it.warmup ? '<div class="prev-t">warm-up</div>' : ''}</div>
      <div class="prev-r mono">${reps}</div>
      <div class="prev-w mono">${w}</div>
    </div>`;
  };

  return obShell(`
    <div class="prev-eyebrow">${h(prog.name)} &middot; ${prog.target} sessions a week</div>
    <h1 class="ob-q">Here is your first one.</h1>
    <p class="ob-sub">Built from what you have just told us &mdash; your kit, your injuries, what you said you can lift. Nothing here is a sample.</p>

    ${items.length ? `<div class="card mt-l" style="padding:6px 14px 10px">
      ${items.map(row).join('')}
    </div>
    <p class="ob-why">It re-cuts itself to the day you actually get: the same session at
      thirty minutes keeps the first movements and drops the rest, and there is a
      three-minute version for the days that fall apart entirely.</p>`
    : `<div class="card mt-l"><p class="small">Your first session will be built once your week is set up.</p></div>`}
  `, { foot:`<button class="btn primary block lg" data-act="ob-next">Looks good</button>` });
};

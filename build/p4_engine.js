...
/* ------------------------------------------------------------
   What an estimated one-rep max is meaningful for.
   ------------------------------------------------------------
   Not a treadmill run. An e1RM is weight times a rep-count factor, and it means
   something only where the weight is load being moved and the reps are
   repetitions of a lift. Feed it a cardio entry and it happily reports that
   your best Treadmill Run is 323 lb, which is exactly what it did — real
   arithmetic on two numbers that meant something else.

   Cardio, carries and mobility are excluded by pattern; held and timed work by
   its resolved load. Bodyweight stays: a weighted pull-up is a loaded lift and
   belongs here, and an unweighted one scores zero on its own anyway. */
function tracksOneRM(item, exIn) {
  const ex = exIn || (item && EX[item.exId]);
  if (!ex) return false;
  if (ex.pattern === 'mobility' || ex.pattern === 'cardio' || ex.pattern === 'carry') return false;
  const load = (item && item.load) || ex.load;
  return load !== 'time' && load !== 'min';
}
...
function isPR(item, st) {
  ...
  if (!tracksOneRM(item, ex)) return false;
...
function applyProgression(sess) {
  sess.exercises.forEach(item => {
    ...
    /* Only where a one-rep max means anything. This block used to sit ABOVE
       the load guard below, so a timed or cardio movement with a number in its
       weight field got an e1RM anyway — a treadmill run reported at 323 lb. */
    if (tracksOneRM(item, ex)) {
      done.forEach(s => {
...
function allPRs() {
  return Object.keys(S.lifts)
    /* Filtered on the way out as well as on the way in. A save written before
       tracksOneRM existed already holds a best for a treadmill run, and this
       makes it disappear without a migration that would have to guess which of
       the stored bests were nonsense. */
    .filter(id => S.lifts[id].best && EX[id] && tracksOneRM(null, EX[id]))

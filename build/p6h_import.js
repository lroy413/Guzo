...
function sheetImport() {
  openSheet(`
    <div class="row between mb"><h2 class="h1">Import a plan</h2></div>
    <p class="small mb">A PDF or a text file with a written programme in it &mdash; the kind a coach sends you,
    ...
    <p class="tiny center mt">Each day becomes one of your routines. Nothing is scheduled and nothing
      already in the app is touched.</p>
  `, { key: 'import' });
}
...
function sheetImportReview() {
  ...
  openSheet(`
    ...
    <button class="btn primary block lg mt" data-act="imp-commit"${c.items ? '' : ' disabled style="opacity:.4"'}>
      ${c.items ? `Build ${c.days} routine${c.days === 1 ? '' : 's'}` : 'Nothing selected'}</button>
    ...
    <p class="tiny center mt-s">Weights come from what you have been lifting, the same as any other
      session. Nothing already in the app is changed.</p>
  `, { key: 'import-review' });
}
...
function commitImport() {
  ...
  IMPORT.days.forEach(d => {
    ...
    items.forEach(it => {
      ...
      const item = r.items[r.items.length - 1];
      item.sets = Math.max(1, Math.min(ROUTINE_LIMITS.sets, it.sets));
      /* The document's unit wins over the catalogue's, the same as anywhere
         else a movement can be counted either way — but only where the app
         allows the choice, or a bench press imported as "60 sec" would turn
         off its own progression. */

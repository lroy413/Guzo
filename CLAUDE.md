...
| `settings` | object | `nutrition, proteinOnly, restMain, restAcc, autoRest, warmup, rpe, lineIdx` — `rpe` is stored as an explicit `false`, never a truthy flag: a save written before it existed has no key, and reading it as merely falsy would take the column off everyone who used it |

...
│   ├── p3_data.js            ← constants, 316 exercises, 6 programmes, copy strings
│   ├── p3b_intake.js         ← onboarding question definitions
│   ├── p3c_form.js           ← 45 hand-built SVG form diagrams
│   ├── p3d_icons.js          ← the drawn icon set. No emoji anywhere user-facing.
...
- **Schema version:** `VERSION = '1.6.0'` — now matching the root `package.json` and `guzo-native/package.json`. Bump all three together; the service worker keys its cache off this one, so a release rotates the cache cleanly

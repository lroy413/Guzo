...
**10b. In a standalone app the body is sized by `100lvh`, and the unit is the whole point.** iOS hands a home-screen app a window the full height of the screen and a layout viewport 62px shorter — measured: `window 402x812` inside an `outer 402x874` at y0, with `svh 812 · lvh 874 · dvh 812 · vh 874`. `position:fixed; inset:0` resolves against the *small* viewport, so every box stopped 62px above the foot of the phone. Only `lvh` reaches the window. It is gated behind `@media (display-mode: standalone)` because in a browser tab `lvh` is the viewport with the address bar retracted, which is the failure that banned `100dvh` from `#app` — a standalone app has no retractable chrome, so there it cannot move. **A desktop browser can prove none of this**: in Chromium, app mode included, `lvh` equals `innerHeight` and the rule is a no-op, `Emulation.setEmulatedMedia` will not set `display-mode` (use `--app=`), and `getComputedStyle` returns the *used* value for a positioned box so `bottom` reads `0px` whether authored `0` or `auto`. The check reads the unit out of the CSSOM, because `svh` and `dvh` both read 812 on that phone and either would apply, match, and change nothing.
